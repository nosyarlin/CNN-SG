...
resnet50_dir_path = "E:/JoejynDocuments/CNN_Animal_ID/Nosyarlin/SBWR_BTNR_CCNR/Results/Big4/Re_0.7_09/trained_model/"
...
resnet50_num_classes = hp.loc[hp['Hyperparameters'] == 'NumClasses', 'Values'].item()
resnet50_dropout = hp.loc[hp['Hyperparameters'] == 'Dropout', 'Values'].item()
resnet50_crop_size = hp.loc[hp['Hyperparameters'] == 'CropSize', 'Values'].item()

resnet50_model, _ = get_model(
    resnet50_archi, resnet50_num_classes, False, False, resnet50_dropout
)
load_checkpoint(resnet50_model_path, model)

resnet50_model.cuda()
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

## Summary of ResNet50 model
summary(resnet50_model, input_size = (3, 299, 299)) #input_size = (channel, input_image_width, input_image_height)
