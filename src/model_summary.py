...
resnet50_model_path = os.path.join(
    resnet50_dir_path, 'archi_resnet50_train_acc_0.898_val_acc_0.927_epoch_15.pth')
resnet50_hp_path = os.path.join(
    resnet50_dir_path, 'hyperparameter_records.csv')

hp = pd.read_csv(resnet50_hp_path)
resnet50_archi = hp.loc[hp['Hyperparameters']
                        == 'Architecture', 'Values'].item()
resnet50_num_classes = int(
    hp.loc[hp['Hyperparameters'] == 'NumClasses', 'Values'].item())
resnet50_dropout = float(
    hp.loc[hp['Hyperparameters'] == 'Dropout', 'Values'].item())
resnet50_crop_size = int(
    hp.loc[hp['Hyperparameters'] == 'CropSize', 'Values'].item())
